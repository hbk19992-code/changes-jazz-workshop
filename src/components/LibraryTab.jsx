...
function FilterChip({ label, active, color, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`px-2 py-1 border text-[11px] tracking-widest uppercase transition-all ${
        active
          ? "bg-[#1a0f08] text-[#fbf4e3] border-[#1a0f08]"
          : "bg-transparent border-[#1a0f08]/30 hover:border-[#1a0f08]"
      }`}
      style={{
...
function ProgressionEditor({ progression, song, onChange, onRemove }) {
  const chordsText = progression.chords.join(" ");

  // Live romans while editing
  const romans = useMemo(
    () =>
      analyzeProgression(progression.chords, song.key, song.scaleType),
    [progression.chords, song.key, song.scaleType]
  );
    ...
    <div className="border border-[#1a0f08]/30 bg-[#f2e8d5] p-3 space-y-2">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            onChange({
              chords: e.target.value.split(/\s+/).filter(Boolean),
              romanOverride: [], // reset overrides when chords change
            })
          }
...
function Label({ children, small }) {
  return (
    <p
      className={`tracking-widest uppercase text-[#6b5b4a] ${
        small ? "text-[9px] mb-0.5" : "text-[10px] mb-1"
      }`}
    >
      {children}
    </p>
